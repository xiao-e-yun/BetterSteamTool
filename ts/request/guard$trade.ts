...
(async () => {
    ...
    if (api === "account") {
        ...
        if (!(req["trade_offers_received"] || req["trade_offers_sent"])) {
            ...
            //處理描述
            const des: [{ appid: number, classid: string, instanceid: string, currency: boolean, background_color: string, icon_url: string, icon_url_large: string, descriptions: [{ type: string, value: string, color: string, }], tradable: boolean, actions: [{ link: string, name: string, }], name: string, name_color: string, type: string, market_name: string, market_hash_name: string, market_actions: [{ link: string; name: string; }], commodity: boolean, market_tradable_restriction: number, marketable: boolean, tags: [{ category: string, internal_name: string, localized_category_name: string, localized_tag_name: string, color: string, }] }
            ] = req["descriptions"]
            des.forEach(item => {//描述
...
/*發出方*/req["trade_offers_sent"].forEach((trade: { "tradeofferid": String, "accountid_other": Number, "message": String, "expiration_time": Number, "trade_offer_state": Number, "items_to_give": [items: object], "is_our_offer": boolean, "time_created": Number, "time_updated": Number, "from_real_time_trade": boolean, "escrow_end_date": Number, "confirmation_method": Number }) => {//交易
...
/*接收方*/req["trade_offers_received"].forEach((trade: { "tradeofferid": String, "accountid_other": Number, "message": String, "expiration_time": Number, "trade_offer_state": Number, "items_to_give": [items: object], "is_our_offer": boolean, "time_created": Number, "time_updated": Number, "from_real_time_trade": boolean, "escrow_end_date": Number, "confirmation_method": Number }) => {//交易
                let status = trade.trade_offer_state
                if (status === 2) {
                    trades.push(trade)
                }
...
async function all_done(main: JQuery<HTMLElement>) {
    ...
    aside.on("click", ".trade_item:not(.checking)", function () {
        ...
        let trade: { "tradeofferid": String, "accountid_other": Number, "message": String, "expiration_time": Number, "trade_offer_state": Number, "items_to_give": [items: object], "items_to_receive": [items: object], "is_our_offer": boolean, "time_created": Number, "time_updated": Number, "from_real_time_trade": boolean, "escrow_end_date": Number, "confirmation_method": Number } = tmp.trades[tradeofferid]
        let other_accinfo: { "steamid": string, "communityvisibilitystate": number, "profilestate": number, "personaname": string, "commentpermission": number, "profileurl": string, "avatar": string, "avatarmedium": string, "avatarfull": string, "avatarhash": string, "lastlogoff": number, "personastate": number, "realname": string, "primaryclanid": string, "timecreated": number, "personastateflags": number, "loccountrycode": string, "locstatecode": string, "loccityid": number, } = tmp.other_accinfo[steamid_other]
    ...
    if (next.length === 0) { main.html(/*html*/`<h2 class="title">目前尚未有交易!</h2><p class="user_info">要是有新的交易，請更新頁面!</p>`).fadeIn() }
    $("#loading").fadeOut()
}

async function post_form(val, trade_id) { //steamid,api,type,trade_id
